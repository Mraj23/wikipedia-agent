...
# ----------------------------------------------------------------------------
# Internal board helpers (operate on a 6x7 numpy array with the same row 0 =
# bottom convention used by ConnectFourEnv._get_board_array).
# ----------------------------------------------------------------------------


def _board_array(env: ConnectFourEnv) -> np.ndarray:
    if env.is_terminal():
        return env._get_board_from_history()  # type: ignore[attr-defined]
    return env._get_board_array()  # type: ignore[attr-defined]


def _landing_row(board: np.ndarray, col: int) -> Optional[int]:
    rows = board.shape[0]
    for r in range(rows):
        if board[r][col] == 0:
            return r
...
def _has_four_through(board: np.ndarray, row: int, col: int, player: int) -> bool:
    """Does the cell at (row, col) participate in a four-in-a-row for `player`?

    Assumes board[row][col] == player on entry. Pure board logic.
    """
...
def _winning_columns(board: np.ndarray, player: int) -> List[int]:
    """Columns in which `player` could play *right now* and complete 4-in-a-row."""
    cols = board.shape[1]
    out: List[int] = []
    for col in range(cols):
        ...
        try:
            if _has_four_through(board, row, col, player):
                out.append(col)
        finally:
            board[row][col] = 0
    return out
