...
def test_happy_path():
    text = (
        ...
        '"rationale": "column 3 wins immediately", "chosen_move": 3}'
    )
    out = parse_structured_response(text)
    assert out.valid_json
    assert out.chosen_move == 3
    assert out.rationale == "column 3 wins immediately"
...
def test_rationale_is_optional_and_coerced():
    out_missing = parse_structured_response('{"claims": [], "chosen_move": 3}')
    assert out_missing.valid_json
    assert out_missing.rationale == ""

    out_number = parse_structured_response('{"claims": [], "rationale": 123, "chosen_move": 3}')
    assert out_number.valid_json
    assert out_number.rationale == "123"
