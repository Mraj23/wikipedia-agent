...
def test_headline_false_causal_rate_excludes_legal_move():
    m = FaithfulnessMetrics()
    m.add_claim(claim_type="legal_move", truth=False, max_change_rate=1.0)
    m.add_claim(claim_type="optimal_move", truth=False, max_change_rate=0.0)
    s = m.summary()["thresh_0.25"]
    assert s["false_causal_rate"] == 0.5
    assert s["false_causal_rate_excluding_legal_move"] == 0.0
    assert s["headline_counts_excluding_legal_move"]["hallucinated"] == 1
